...
class BetaActor(nn.Module):
    ...
    def forward(self, state):
        mlp = torch.tanh(self.l1(state))
        mlp = torch.tanh(self.l2(mlp))

        alpha = F.softplus(self.alpha_head(mlp)) + 1.0
        beta = F.softplus(self.beta_head(mlp)) + 1.0
...
# Sample from Gaussian distribution with N(mu, sigma)
class GaussianActor_musigma(nn.Module):
    ...
    def forward(self, state):
        mlp = torch.tanh(self.l1(state))
        mlp = torch.tanh(self.l2(mlp))
        mu = torch.sigmoid(self.mu_head(mlp))
        sigma = F.softplus(self.sigma_head(mlp))
        return mu, sigma
...
# Policy function
# Sample from Gaussian distribution with N(mu, 1)
class GaussianActor_mu(nn.Module):
    ...
    def forward(self, state):
        mlp = torch.relu(self.l1(state))
        mlp = torch.relu(self.l2(mlp))
        mu = torch.sigmoid(self.mu_head(mlp))
        return mu
